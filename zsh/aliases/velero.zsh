...
set-velero-pr-review-users() {
  ...
  local maintainers
  maintainers=${(j: :)${(@f)$(curl -s --fail --connect-timeout 10 "$maintainers_url" | grep -E '@[a-zA-Z0-9_-]+' | grep -v -i 'emeritus' | sed -E 's/.*@([a-zA-Z0-9_-]+).*/\1/' | sort -u)}}
...
set-oadp-pr-review-users() {
  ...
  local owners
  owners=${(j: :)${(@f)$(curl -s --fail --connect-timeout 10 "$owners_url" | grep -E '^\s*-\s+[a-zA-Z0-9_-]+\s*$' | sed -E 's/^\s*-\s+([a-zA-Z0-9_-]+)\s*$/\1/' | sort -u)}}
