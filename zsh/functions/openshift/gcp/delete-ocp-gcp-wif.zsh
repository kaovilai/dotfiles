...
znap function delete-ocp-gcp-wif-dir() {
    ...
    if [[ $dir_basename =~ ^([0-9]{8})-gcp-wif(-[0-9]+)?$ ]]; then
        ...
        if [[ -n "$extracted_suffix" ]]; then
            ...
            local ocp_create_dir="$OCP_MANIFESTS_DIR/$extracted_date-gcp-wif$extracted_suffix"
            local cluster_name="tkaovila-$extracted_date-wif$extracted_suffix"
            echo "Using directory path: $ocp_create_dir"
            echo "Using cluster name: $cluster_name"
        else
            # Use local variables to avoid polluting the global environment
            local ocp_create_dir="$OCP_MANIFESTS_DIR/$extracted_date-gcp-wif"
            local cluster_name="tkaovila-$extracted_date-wif"
            echo "Using directory path: $ocp_create_dir"
            echo "Using cluster name: $cluster_name"
        fi
        ...
        OCP_CREATE_DIR="$ocp_create_dir" CLUSTER_NAME="$cluster_name" delete-ocp-gcp-wif "$cluster_name"
        ...
        local fallback_dir="$OCP_MANIFESTS_DIR/$fallback_date-gcp-wif"
        local fallback_cluster="tkaovila-$fallback_date-wif"
        ...
        OCP_CREATE_DIR="$fallback_dir" CLUSTER_NAME="$fallback_cluster" delete-ocp-gcp-wif "$fallback_cluster"
