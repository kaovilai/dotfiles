...
znap function create-ocp-<provider>() {
    ...
    local stream=$(prompt-release-stream)
    local RELEASE_IMAGE=$(get-release-image "$stream" "<architecture>")
    ...
    if ! $OPENSHIFT_INSTALL create cluster --dir $OCP_CREATE_DIR --log-level=info; then
        cleanup-on-failure "$OCP_CREATE_DIR" "$CLUSTER_NAME" "<provider>"
        return 1
    fi
...
- `get-openshift-install()`: Find appropriate openshift-install binary
- `validate-env-vars()`: Validate required environment variables
- `prompt-release-stream()`: Interactive release stream selection
- `get-release-image()`: Get release image for stream/architecture
- `handle-registry-login()`: Handle registry authentication
- `update-pull-secret-with-podman()`: Update pull secret with podman creds
- `generate-unique-cluster-name()`: Generate unique cluster names
- `cleanup-on-failure()`: Cleanup resources on failure
- `create-install-config-header()`: Standard install-config header
- `add-credentials-to-install-config()`: Add pull secret and SSH key