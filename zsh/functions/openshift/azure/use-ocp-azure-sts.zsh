...
znap function use-ocp-azure-sts(){
    ...
    if [[ $1 == "help" ]]; then
        ...
        echo "Copy kubeconfig from the Azure STS cluster to the default location (~/.kube/config)"
        echo ""
        echo "Arguments:"
        echo "  directory-suffix    Optional suffix if a numbered directory was created (e.g., 1, 2)"
        echo ""
        echo "Examples:"
        echo "  use-ocp-azure-sts         # Use the default installation directory"
        echo "  use-ocp-azure-sts 2       # Use the installation directory with suffix '-2'"
        echo ""
        return 0
    fi
    ...
    local OCP_CREATE_DIR="$OCP_MANIFESTS_DIR/$TODAY-azure-sts$SUFFIX"
...
znap function use-ocp-azure-sts-dir() {
    ...
    # Check if help is requested
    if [[ $1 == "help" ]]; then
        echo "Usage: use-ocp-azure-sts-dir DIRECTORY_PATH"
        echo "Use an OpenShift cluster on Azure with STS based on the directory path"
        echo ""
        echo "Parameters:"
        ...
        echo ""
        echo "Example:"
        echo "  use-ocp-azure-sts-dir ~/OCP/manifests/20250410-azure-sts"
        return 0
    fi
