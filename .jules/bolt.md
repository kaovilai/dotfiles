...
## 2025-01-01 - Avoid Manual compinit with Znap
**Learning:** The Znap plugin manager automatically handles `compinit` and comp dumps out of the box. Manually calling `compinit` (e.g., in `zsh/znap.zsh`) degrades shell startup performance due to redundant execution.
**Action:** Rely on Znap's built-in completion initialization and avoid manually calling `compinit` in Znap-managed configurations.
