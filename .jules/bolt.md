...
## 2024-11-20 - Znap automatically handles compinit
**Learning:** The Znap plugin manager automatically handles `compinit` and comp dumps out of the box. Manually calling `compinit` (e.g., in `.zshrc` or `zsh/znap.zsh`) degrades shell startup performance due to redundant execution.
**Action:** Do not manually call `compinit` when using Znap. Removing the redundant call significantly improved shell startup performance in this codebase.
